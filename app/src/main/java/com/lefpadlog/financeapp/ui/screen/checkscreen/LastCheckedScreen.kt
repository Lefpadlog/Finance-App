...
fun LastCheckedScreen(navController: NavController) {
    ...
    val previousSelectedIndex = remember { mutableIntStateOf(0) }

    val amountList = remember { AppDatabase.paymentMethods.getAll().map { it.amount.toString() }.toMutableList() }
    val amountFieldValue = remember { mutableStateOf(amountList[selectedIndex.intValue]) }

    LaunchedEffect(selectedIndex.intValue) {
        amountList[previousSelectedIndex.intValue] = amountFieldValue.value
        amountFieldValue.value = amountList[selectedIndex.intValue]
        previousSelectedIndex.intValue = selectedIndex.intValue
    }
    ...
    Column(
        ...
        Row(
            ...
            SelectedPaymentMethodBox(paymentMethodList[selectedIndex.intValue], amountFieldValue)
